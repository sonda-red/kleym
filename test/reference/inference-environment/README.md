...
service account, serving workload, and `InferencePool` with stable names and
selectors.
...
2. Apply an `InferenceIdentityBinding` that anchors to `reference-pool`.
...
applied by the test or demo layer, not by this directory.
