...
variant in the SPIFFE ID. Mandatory workload selectors enforce the namespace,
service account, complete pool selector, and fixed `identity.kleym.sonda.red/variant` label.

For distinct SPIFFE IDs in one namespace and service account, Kleym accepts only
one structural exclusivity proof: different values of the fixed variant label.
Reused variants and duplicate SPIFFE IDs conflict. The
